...
(() => {
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    setConfig(config) {
      ...
      this._config.devices = this._config.devices.map(d => {
        ...
        if (!Number.isFinite(Number(copy.scale))) copy.scale = 1;
        return copy;
      });
  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _render(){
      ...
      this.shadowRoot.querySelectorAll('[data-device-scale]').forEach(el=>el.addEventListener('input',e=>{
        const i=Number(e.target.dataset.deviceScale);
        this._config.devices[i].scale = Number(e.target.value)/100;
        this._emit(false);
      }));
    ...
    _layoutSpecial(kind,label,icon,pos,dx,dy){ const p=pos && Number.isFinite(Number(pos.x)) && Number.isFinite(Number(pos.y)) ? pos : {x:dx,y:dy}; return `<div class="layout-node special" data-layout-kind="${kind}" style="left:${p.x}%;top:${p.y}%;--device-scale:${Number(d.scale || 1)}"><div class="ln-top">${icon} ${label}</div><div class="ln-pos">${Number(p.x).toFixed(1)}% × ${Number(p.y).toFixed(1)}%</div></div>`; }
