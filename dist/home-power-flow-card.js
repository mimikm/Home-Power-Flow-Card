...
(() => {
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    _flowDirection(a, b, va, vb) {
      ...
      // 0.6.x topology:
      // Inverter is only a junction. It never decides direction.
      // Direction is determined by the source devices:
      // solar -> inverter, battery charge/discharge, grid import/export.

      let reverse = false;

      if (ta === 'grid' || tb === 'grid') {
        const grid = ta === 'grid' ? Number(va) : Number(vb);
        // positive grid value = import, negative = export
        reverse = ta === 'grid' ? grid < 0 : grid > 0;
      } else if (ta === 'battery' || tb === 'battery') {
        const bat = ta === 'battery' ? Number(va) : Number(vb);
        // positive battery power = charging (inverter -> battery)
        reverse = ta === 'battery' ? bat < 0 : bat > 0;
      } else if (ta === 'solar' || tb === 'solar') {
        // solar always feeds inverter/load
        reverse = ta === 'solar' ? false : true;
      } else if (ta === 'inverter' || tb === 'inverter') {
        // inverter as junction: use the non-inverter device
        const other = ta === 'inverter' ? tb : ta;
        if (other === 'load' || other === 'house' || other === 'ev') reverse = ta === 'inverter' ? false : true;
      }
  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _render(){
      ...
      </style><div class="wrap"><h3>Home Power Flow V0.6.0.1</h3><div class="hint">Bidirectional power flow from live positive/negative values, dotted connections, single moving power dot, invertible device direction, dynamic flow colors and draggable layout. Entity IDs are shown in full below each picker.</div>
