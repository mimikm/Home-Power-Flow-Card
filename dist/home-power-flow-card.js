...
(() => {
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    setConfig(config) {
      ...
      if (!Array.isArray(this._config.devices)) this._config.devices = [];
       this._config.devices = this._config.devices.map(d => ({ scale: 1, ...d }));
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          * { box-sizing:border-box; }
          ...
          .node { position:absolute; transform:translate(-50%,-50%) scale(var(--device-scale,1)); width:clamp(135px,13vw,205px); min-height:74px; padding:11px 13px; border-radius:15px; z-index:10; background:linear-gradient(145deg,rgba(9,25,40,.87),rgba(15,30,44,.73)); border:1px solid rgba(255,255,255,.17); box-shadow:0 8px 22px rgba(0,0,0,.32); backdrop-filter:blur(10px); }
    ...
    _deviceHTML(d, i, p) {
      ...
      return `<div class="node ${esc(d.type || 'load')}" data-device-index="${i}" data-entity-id="${esc(d.power_entity || '')}" style="--device-scale:${Number(d.scale || 1)};left:${p.x}%;top:${p.y}%" title="${esc(d.power_entity ? 'Open ' + d.power_entity : '')}" style="left:${p.x}%;top:${p.y}%;--device-scale:${Number(d.scale || 1)}"><div class="top"><span class="icon">${esc(d.icon || ICONS[d.type] || '⚙️')}</span><span class="name">${esc(d.name || LABELS[d.type] || 'Device')}</span></div><div class="power">${esc(powerText)}</div><div class="secondary">${secHtml}</div></div>`;
    }
  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _render(){
      ...
      </style><div class="wrap"><h3>Home Power Flow V0.5.8</h3><div class="hint">Bidirectional power flow from live positive/negative values, dotted connections, single moving power dot, invertible device direction, dynamic flow colors and draggable layout. Entity IDs are shown in full below each picker.</div>
