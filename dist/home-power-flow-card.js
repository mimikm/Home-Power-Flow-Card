  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    static getStubConfig() {
      return {
        ...
        background_day_position: { x: 50, y: 50, zoom: 100 },
        background_night_position: { x: 50, y: 50, zoom: 100 },
        sun_entity: 'sun.sun',
        devices: [
    ...
    backgroundPositionStyle(slot='day') {
      const key = slot === 'night' ? 'background_night_position' : 'background_day_position';
      const p = this._config?.[key] || {x:50,y:50,zoom:100};
      return {
        size: `${Math.max(50, Math.min(250, Number(p.zoom)||100))}%`,
        position: `${Number(p.x)||50}% ${Number(p.y)||50}%`
      };
    }
    ...
    _deviceHTML(d, i, p) {
      ...
      return `<div class="node ${esc(d.type || 'load')}" data-device-index="${i}" data-entity-id="${esc(d.power_entity || '')}" title="${esc(d.power_entity ? 'Open ' + d.power_entity : '')}" style="left:${p.x}%;top:${p.y}%;--device-scale:${Number(d.scale||1)}"><div class="top"><span class="icon">${esc(d.icon || ICONS[d.type] || '⚙️')}</span><span class="name">${esc(d.name || LABELS[d.type] || 'Device')}</span></div><div class="power">${esc(powerText)}</div><div class="secondary">${secHtml}</div></div>`;
    }
...
<h3>Background crop controls</h3>
<div class="hint">Adjust image framing without editing the image. Useful for solar/house illustrations.</div>
<div class="row">
<div class="field"><label>Background X position</label><input type="range" min="0" max="100" data-bg-crop="x" value="${esc((c.background_day_position?.x)||50)}"></div>
<div class="field"><label>Background Y position</label><input type="range" min="0" max="100" data-bg-crop="y" value="${esc((c.background_day_position?.y)||50)}"></div>
<div class="field"><label>Background zoom</label><input type="range" min="50" max="200" data-bg-crop="zoom" value="${esc((c.background_day_position?.zoom)||100)}"></div>
</div>
<h3>Visual layout</h3><div class="hint">Drag the device boxes on the template to place them exactly where you want. Positions are saved automatically. New devices without a saved position use the automatic layout.</div><div class="layout-editor" id="layout-editor"><div class="layout-bg"></div>${(c.devices||[]).map((d,i)=>this._layoutNode(d,i)).join('')}${this._layoutSpecial('weather','Weather','🌤️',c.weather_position,82,10)}${this._layoutSpecial('stats','Daily Stats','📊',c.stats_position,17,86)}<button class="btn secondary-btn" id="reset-layout" style="position:absolute;right:10px;bottom:10px;z-index:5">Reset positions</button></div><button class="btn secondary-btn" id="reset-layout">↺ Reset positions to automatic</button>
      <h3>Device box scaling</h3><div class="hint">Select device size in the list below. Changes are saved per device.</div><h3>Devices</h3>${(c.devices||[]).map((d,i)=>this._device(d,i)).join('')}<button class="btn" id="add">＋ Add device</button>
      ...
      this.shadowRoot.querySelectorAll('[data-device-scale]').forEach(el=>el.addEventListener('input',e=>{const i=Number(e.target.dataset.deviceScale);this._config.devices[i].scale=(Number(e.target.value)||100)/100;this._emit(false);}));
    ...
    _layoutNode(d,i){ const p=d.position && Number.isFinite(Number(d.position.x)) && Number.isFinite(Number(d.position.y)) ? d.position : this._autoPreviewPosition(d,i); return `<div class="layout-node" data-layout-kind="device" data-layout-index="${i}" style="left:${p.x}%;top:${p.y}%;--device-scale:${Number(d.scale||1)}"><div class="ln-top">${esc(d.icon || ICONS[d.type] || '⚙️')} ${esc(d.name || LABELS[d.type] || 'Device')}</div><div class="ln-pos">${Number(p.x).toFixed(1)}% × ${Number(p.y).toFixed(1)}%</div><div class="ln-pos">Size ${Math.round(Number(d.scale||1)*100)}%</div></div>`; }
    _layoutSpecial(kind,label,icon,pos,dx,dy){ const p=pos && Number.isFinite(Number(pos.x)) && Number.isFinite(Number(pos.y)) ? pos : {x:dx,y:dy}; return `<div class="layout-node special" data-layout-kind="${kind}" style="left:${p.x}%;top:${p.y}%;--device-scale:${Number(d.scale||1)}"><div class="ln-top">${icon} ${label}</div><div class="ln-pos">${Number(p.x).toFixed(1)}% × ${Number(p.y).toFixed(1)}%</div><div class="ln-pos">Size ${Math.round(Number(d.scale||1)*100)}%</div></div>`; }
