...
(() => {
  ...
  function flowColor(a, b, reverse, active, customColors = {}) {
    ...
    // Flow colour belongs to the actual energy device, never the inverter hub.
    // Direction changes must not change the selected colour.
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    _flows(devices, pos) {
      ...
      return edges.map(([a,b,dir],idx)=>{
        ...
        if(da.invert_flow) reverse=!reverse;
        if(db.invert_flow) reverse=!reverse;
  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _render(){
      ...
      <h3>Flow colours</h3><div class="hint">Choose the colour used by the dotted flow path and its travelling power dot. Changes apply immediately.</div><div class="row flow-colours">${this._colorField('solar','Solar')}${this._colorField('battery','Battery')}${this._colorField('gateway','Gateway')}${this._colorField('house','House / Load')}${this._colorField('grid','Grid')}${this._colorField('ev','EV Charger')}${this._colorField('load','Extra Load')}${this._colorField('neutral','Inactive')}</div>
