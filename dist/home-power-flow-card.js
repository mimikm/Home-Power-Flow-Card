...
(() => {
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    _deviceHTML(d, i, p) {
      ...
      return `<div class="node ${esc(d.type || 'load')}" data-device-index="${i}" data-entity-id="${esc(d.power_entity || '')}" title="${esc(d.power_entity ? 'Open ' + d.power_entity : '')}" style="left:${p.x}%;top:${p.y}%;transform:translate(-50%,-50%) scale(${Number(d.scale||1)})"><div class="top"><span class="icon">${esc(d.icon || ICONS[d.type] || '⚙️')}</span><span class="name">${esc(d.name || LABELS[d.type] || 'Device')}</span></div><div class="power">${esc(powerText)}</div><div class="secondary">${secHtml}</div></div>`;
    }
  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _render(){
      ...
      </style><div class="wrap"><h3>Home Power Flow V0.5.9</h3><div class="hint">Bidirectional power flow from live positive/negative values, dotted connections, single moving power dot, invertible device direction, dynamic flow colors and draggable layout. Entity IDs are shown in full below each picker.</div>
