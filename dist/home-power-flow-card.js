...
(() => {
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    static getStubConfig() {
      return {
        ...
        sun_entity: 'sun.sun',
        devices: [
          { type: 'solar', name: 'Solar PV', power_entity: '', scale: 1 },
          { type: 'inverter', name: 'Inverter 1', power_entity: '', temp_entity: '', scale: 1 },
          { type: 'battery', name: 'Battery 1', power_entity: '', soc_entity: '', voltage_entity: '', temp_entity: '', scale: 1 },
  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _layoutNode(d,i){ const p=d.position && Number.isFinite(Number(d.position.x)) && Number.isFinite(Number(d.position.y)) ? d.position : this._autoPreviewPosition(d,i); const scale=Math.round((Number(d.scale)||1)*100); return `<div class="layout-node" data-layout-kind="device" data-layout-index="${i}" style="left:${p.x}%;top:${p.y}%"><div class="ln-top">${esc(d.icon || ICONS[d.type] || '⚙️')} ${esc(d.name || LABELS[d.type] || 'Device')}</div><div class="ln-pos">${Number(p.x).toFixed(1)}% × ${Number(p.y).toFixed(1)}%</div><div class="ln-scale">Size ${scale}%</div></div>`; }
