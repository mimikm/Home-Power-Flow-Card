  ...
  class HomePowerFlowEditor extends HTMLElement {
    ...
    _render(){
      ...
      this.shadowRoot.querySelectorAll('.stat-sort-item').forEach(item=>{
        item.addEventListener('dragstart',()=>{draggedStat=Number(item.dataset.statIndex);item.classList.add('dragging');});
        item.addEventListener('dragend',()=>{item.classList.remove('dragging');draggedStat=null;});
        item.addEventListener('dragover',e=>e.preventDefault());
        item.addEventListener('drop',e=>{e.preventDefault();const t=Number(item.dataset.statIndex);if(draggedStat===null||t===draggedStat)return;const a=this._config.statistics.entities;const m=a.splice(draggedStat,1)[0];a.splice(t,0,m);this._emit(false);this._render();});
      });
    ...
    _statsEditorHTML(){
      ...
      return list.map((r,i)=>`<div class="device stat-sort-item" draggable="true" data-stat-index="${i}">
      <div class="device-head"><span class="stat-drag-handle" title="Drag to reorder">⠿</span><span>📊 Statistic ${i+1}</span><button data-remove-stat="${i}">×</button></div>
      <div class="row">
      <div class="field"><label>Name</label><input data-stat-field="name" data-index="${i}" value="${esc(r.name||'')}"></div>
      <div class="field full"><label>Material Design Icon</label><ha-icon-picker data-stat-icon data-index="${i}" value="${esc(r.icon||'mdi:chart-line')}"></ha-icon-picker></div>
      ...
      </div></div>`).join('') || '<div class="small">No statistics added.</div>';
    }
