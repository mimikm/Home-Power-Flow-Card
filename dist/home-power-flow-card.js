...
(() => {
  ...
  class HomePowerFlowCard extends HTMLElement {
    ...
    // Maximum card width in px (0 = no limit). Combined in CSS with a cap
    // that keeps the whole card inside the screen height.
    _maxWidthCss() {
      const raw = Number(this._config?.max_width);
      if (Number.isFinite(raw) && raw === 0) return '100000px';
      return (Number.isFinite(raw) && raw >= 300 ? Math.round(raw) : 1440) + 'px';
    }
      ...
      this.shadowRoot.innerHTML = `
        <style>
          ...
          * { box-sizing:border-box; }
          .card { position:relative; width:100%; max-width:min(var(--hpf-max-width,1440px), calc((100vh - 96px) * 1.5)); margin:0 auto; aspect-ratio: 3 / 2; overflow:hidden; border-radius:22px; color:#fff; font-family:var(--primary-font-family,Arial,sans-serif); background:#101820; box-shadow:0 12px 40px rgba(0,0,0,.28); }
